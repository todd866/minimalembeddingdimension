...
The cylinder $S^1 \times \R$ \emph{can} be embedded in $\R^2$---for example, as a spiral via $(\theta, \tau) \mapsto (e^\tau \cos\theta, e^\tau \sin\theta)$. The obstruction in Theorem~\ref{thm:main} arises not from the cylinder topology alone, but from the \emph{phase-preserving} constraint (Eq.~\ref{eq:phase-preserving}), which requires the image to depend only on phase $\theta$, effectively collapsing the cylinder fibers onto a circle. Spiral embeddings violate this constraint by encoding meta-time in the radial coordinate.
\end{remark}
...
\begin{enumerate}
    \item[(i)] \textbf{(Non-existence in $\R^2$)} Any continuous \emph{phase-preserving} map $\pi_2: \M \to \R^2$---meaning there exists a simple closed curve $C \subset \R^2$ and a homeomorphism $f: S^1 \to C$ such that
    \begin{equation}\label{eq:phase-preserving}
        \pi_2(\gamma(t)) = f(\theta(\gamma(t)))
    \end{equation}
    for all $t$---necessarily has self-intersections: $\pi_2 \circ \gamma$ is not injective.
...
The phase-preserving condition \eqref{eq:phase-preserving} excludes embeddings that use ``hidden dimensions'' to encode meta-time. For instance, a spiral $(\cos\theta, \sin\theta, r(\tau))$ in $\R^3$ projected to $\R^2$ as $(r\cos\theta, r\sin\theta)$ avoids self-intersections by encoding $\tau$ in the radius---but this is \emph{not} phase-preserving, since the image point depends on $\tau$, not just $\theta$. The theorem says: if your 2D representation genuinely factors through phase alone, collisions are unavoidable.
\end{remark}

\begin{proof}[Proof of (i)]
Since $\theta \circ \gamma$ covers $S^1$ at least $n \geq 2$ times and $\tau$ is strictly monotone, there exist distinct times $t_1 \neq t_2$ with $\theta(\gamma(t_1)) = \theta(\gamma(t_2))$ (same phase).

By the phase-preserving condition \eqref{eq:phase-preserving}:
\[
\pi_2(\gamma(t_1)) = f(\theta(\gamma(t_1))) = f(\theta(\gamma(t_2))) = \pi_2(\gamma(t_2)).
\]
...
\textbf{Minimality within the phase-preserving class:} By Lemma~\ref{lem:cylinder}, the trajectory has intrinsic cylinder structure $S^1 \times \R$. A phase-preserving embedding (Eq.~\ref{eq:phase-preserving}) constrains the image of the $S^1$ factor to a simple closed curve $C$ whose points depend only on phase. The full trajectory then lives in a tubular neighbourhood of $C$, and preserving injectivity requires at least one transverse dimension for the $\R$ factor (meta-time). Thus $k = 3$ is minimal \emph{among phase-preserving embeddings}: two dimensions to accommodate the closed curve, one for the monotone coordinate. (Without phase-preservation, spiral embeddings into $\R^2$ are possible; see Remark after Lemma~\ref{lem:cylinder}.)
\end{proof}
...
\textbf{Numerical proxy:} In Figure~\ref{fig:fisher}, we illustrate the rank drop using eigenvalues of the empirical covariance matrix as a proxy for the Fisher metric spectrum. This is appropriate for our toy model where the covariance structure tracks the geometric structure; in general statistical settings, the Fisher metric must be computed from the model likelihood.
\end{proposition}
...
\textbf{Projection to $\R^2$:} If we project via $\pi_2(\mu, \sigma) = (\mu, 0)$ (discarding $\sigma$), the trajectory collapses to $[-A, A]$. Distinct times with the same $\mu$ (i.e., same phase $\theta$) become identical, producing self-intersections. The induced metric has a null direction: the projection Jacobian kills the $\sigma$ (equivalently, $\tau$) direction. Note that this rank drop is not due to the intrinsic Fisher metric becoming singular as $\sigma \to 0$; it is due to the projection discarding $\sigma$ entirely, so variations in meta-time become undetectable regardless of $\sigma$.
